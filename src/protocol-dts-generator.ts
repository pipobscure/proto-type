...
import type {IProtocol, Protocol as P} from './protocol-schema'

let protocolName = 'Protocol';
const protocolDomains: P.Domain[] = process.argv.slice(2).reduce((domains: P.Domain[], filename: string) =>{
    const proto: IProtocol = JSON.parse(fs.readFileSync(filename, 'utf-8'));
    protocolName = proto.name ?? protocolName;
    domains.push(...proto.domains);
    return domains;
}, []);
...
const emitPublicDocDeclaration = () => {
    emitLine('/**');
    emitLine(` * The ${protocolName} Protocol.`);
    emitLine(' * @public')
    emitLine(' */')
}
...
// Main
const destProtocolFilePath = path.resolve('protocol.d.ts');
...
const destMappingFilePath = path.resolve('mapping.d.ts');
...
const destApiFilePath = path.resolve('proxy.d.ts');
